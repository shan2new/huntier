export {}
// Runs in an offscreen popup tab. Invisibly scrolls and extracts data without user impact.

async function sleep(ms: number) { return new Promise((r) => setTimeout(r, ms)) }

function extractStructured() {
  const data: any = { title: '', company: {}, location: {}, sections: [] as string[] }
  // JSON-LD JobPosting
  ...
  for (const s of scripts) {
    try {
      ...
      for (const n of nodes) {
        if (!n) continue
        const types = (Array.isArray(n['@type']) ? n['@type'] : [n['@type']]).map((x:any)=>String(x).toLowerCase())
  ...
  // Basic DOM text accumulation for AI hints
...
async function crawlAndExtract() {
  ...
  ;(globalThis as any).chrome?.runtime?.sendMessage?.({ type: 'huntier:crawl-result', extracted })
}

crawlAndExtract().catch(() => {
  ;(globalThis as any).chrome?.runtime?.sendMessage?.({ type: 'huntier:crawl-result', extracted: null })
})
