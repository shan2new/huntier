...
export function Chat({
  ...
  const handleStop = useCallback(() => {
    ...
    const latestMessages: Array<any> = [...messagesRef.current]
    const lastAssistantMessage = (() => {
      for (let i = latestMessages.length - 1; i >= 0; i--) {
        const m: any = latestMessages[i]
    ...
    if (lastAssistantMessage.toolInvocations) {
      const updatedToolInvocations = (lastAssistantMessage.toolInvocations as Array<any>).map(
        (toolInvocation: any) => {
